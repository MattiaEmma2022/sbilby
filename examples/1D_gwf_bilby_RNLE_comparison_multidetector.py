...
ifo_data = bilby.gw.detector.InterferometerList(interferometers)
ifo_signal = bilby.gw.detector.InterferometerList(interferometers)
ifo_noise = bilby.gw.detector.InterferometerList(interferometers)

for i in range(len(interferometers)):
    ifo_data[i].set_strain_data_from_frame_file('Test_aligned_4096.gwf',channel='H1:1126259642_SIM' ,sampling_frequency=sampling_frequency, duration= duration, start_time=start_time, buffer_time=4)
    ifo_signal[i].set_strain_data_from_frame_file('Test_aligned_noise_4096.gwf',channel='H1:1126259642_SIM' ,sampling_frequency=sampling_frequency, duration= duration, start_time=start_time, buffer_time=4)
    ifo_noise[i].set_strain_data_from_frame_file('Test_aligned_noise_4096.gwf',channel='H1:1126259642_SIM' ,sampling_frequency=sampling_frequency, duration= duration, start_time=start_time, buffer_time=4)
...
yobs=[]
for ifo in ifo_data:
    yobs.append(ifo.whitened_time_domain_strain)
...
labels=[]
for i in interferometers:
    label=f"N{num_simulations}_fs{sampling_frequency}_seed{args.rseed}_R{args.repeat}_TL{args.time_lower}_TU{args.time_upper}_{i}"   
    labels.append(label)
